...
# FRAME_TIMEOUT = 10  # Reduce timeout to detect stream failures quickly
...
# active_streams = manager.dict()  # Store PIDs, not process objects
# section_streams = manager.dict()
# logger = logging.getLogger(__name__)
...
#     logger.info(f"Starting camera process for {camera_id} with URL: {camera_url}")
    
#     process = mp.Process(target=stream_camera_ffmpeg, args=(camera_id, str(camera_url)), daemon=True)
#     process.start()
#     active_streams[camera_id] = process.pid  # Store PID instead of process object

# def stream_camera_ffmpeg(camera_id, camera_url):
#     """Handles streaming of a camera feed via FFmpeg."""
#     logger.info(f"Starting FFmpeg stream for camera {camera_id} at {camera_url}")
...
#                 logger.error(f"FFmpeg process for camera {camera_id} exited unexpectedly.")
#                 restart_camera_stream(camera_id)
#                 return
            
#             raw_frame = process.stdout.read(FRAME_SIZE)
#             if not raw_frame or len(raw_frame) != FRAME_SIZE:
#                 logger.warning(f"Incomplete frame received from camera {camera_id}. Retrying...")
...
#     logger.info(f"Cleaning up camera {camera_id} stream.")
    
#     if camera_id in active_streams:
#         pid = active_streams.pop(camera_id, None)
#         if pid:
#             subprocess.call(["kill", "-9", str(pid)])  # Kill the process
...
#     while True:
#         buffer = frame_buffers.get(camera_id, None)
...
#             if sec_id != pk:
#                 for camera_id in section_streams.pop(sec_id, []):
#                     cleanup_camera_stream(camera_id)
...
#             section_streams[pk].append(camera.id)
...
#     while True:
#         frame = cache.get(f"camera_stream:{camera_id}")
#         if frame:
#             yield (b'--frame\r\n'
#                    b'Content-Type: image/jpeg\r\n'
#                    b'Content-Length: ' + f"{len(frame)}".encode() + b'\r\n'
...
#     print(f"DEBUG: Checking Camera {camera_id}")
#     print(f"DEBUG: Active Section in Cache: {current_section}")
#     print(f"DEBUG: Camera's Section: {camera.section.id}")

#     if current_section is None or str(current_section) != str(camera.section.id):
#         print("DEBUG: Access Denied - Camera is not in the active section")
...
#         stream_camera_ffmpeg.apply_async(args=[camera.id, camera.get_rtsp_url(), pk])
#         cache.set(f"camera_active:{camera_id}", True)
...
#     def retrieve(self, request, pk=None):
#         """Handles section-based camera streaming requests."""
...
#         print(f"DEBUG: Before Update - Previous Active Section: {previous_section}")

#         # If section is changed, stop previous section's cameras
#         if previous_section and str(previous_section) != str(pk):
#             previous_cameras = Camera.objects.filter(section_id=previous_section, is_active=True)
#             for camera in previous_cameras:
#                 cache.delete(f"camera_active:{camera.id}")  # Remove old camera flag
#                 cache.delete(f"camera_stream:{camera.id}")  # Clear old frames

#             # Set new active section in cache
#             cache.set("active_section", str(pk))
#             print(f"DEBUG: After Update - New Active Section Set: {cache.get('active_section')}")

#         # Start streaming for current section cameras
#         for camera in cameras:
#             active_stream_urls[camera.id] = f"/api/video_feed/{camera.id}/"
#             if not cache.get(f"camera_active:{camera.id}"):
#                 stream_camera_ffmpeg.apply_async(args=[camera.id, camera.get_rtsp_url(), pk])
#                 cache.set(f"camera_active:{camera.id}", True)
